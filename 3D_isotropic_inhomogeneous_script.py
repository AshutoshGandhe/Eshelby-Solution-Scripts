...
'''
def find_esp_star():
    deltaCijkl = np.zeros((3,3,3,3))
    for i in range(3):
        for j in range(3):
            for k in range(3):
                for l in range(3):
                    deltaCijkl[i][j][k][l] = Cijkl[i][j][k][l] - Cijkl_star[i][j][k][l]
    A = 
...
def calc_interior():
    ...
    #print("EPSILON STAR STAR")
    #print(epsilon_star_star)
    matrix = np.array([[elem[0] for elem in row] for row in epsilon])
    print("EPSILON")
    print(matrix)
        ...
        for j in range(3):
            for k in range(3):
                for l in range(3):
                    ...
                    for m in range(3):
                        for n in range(3):
                            tmp += Sijkl[k][l][m][n]*epsilon_star_star[m][n]
                    sigmaij[i][j] += Cijkl_star[i][j][k][l] * (tmp - epsilon_p[k][l])
    ...
    return [sigmaij[0][0], sigmaij[1][1], sigmaij[2][2]]
...
def calc_exterior():
    ...
    sigmaij_out[0][0] = (E / ((1 + nu) * (1 - 2 * nu))) * ((1 - nu) * epsilon_out[0][0] + nu * epsilon_out[1][1] + nu * epsilon_out[2][2])
    sigmaij_out[1][1] = (E / ((1 + nu) * (1 - 2 * nu))) * (nu * epsilon_out[0][0] + (1 - nu) * epsilon_out[1][1] + nu * epsilon_out[2][2])
    sigmaij_out[2][2] = (E / ((1 + nu) * (1 - 2 * nu))) * (nu * epsilon_out[0][0] + nu * epsilon_out[1][1] + (1 - nu) * epsilon_out[2][2])
    ...
    return [sigmaij_out[0][0], sigmaij_out[1][1], sigmaij_out[2][2]]
...
E_star = 220  # Young's Modulus
nu_star = 0.35  # Poisson's Ratio
...
for i in range(3):
    for j in range(3):
        for k in range(3):
            for l in range(3):
                Cijkl[i][j][k][l] = lamda * deltaij[i][j] * deltaij[k][l] + mu * (deltaij[i][k] * deltaij[j][l] + deltaij[i][l] * deltaij[j][k])
...
for i in range(3):
    for j in range(3):
        for k in range(3):
            for l in range(3):
                Cijkl_star[i][j][k][l] = lamda_star * deltaij[i][j] * deltaij[k][l] + mu_star * (deltaij[i][k] * deltaij[j][l] + deltaij[i][l] * deltaij[j][k])
